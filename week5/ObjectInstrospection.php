...
function get_lineage($object)
{
    ...
    } else $lineage = array(get_class($object));
    return $lineage;
}
...
function get_child_classes($object)
{
    ...
    foreach ($classes as $class) {
        if (substr($class, 0, 2) == '_ _') continue;
        if (get_parent_class($class) == get_class($object)) $children = $class;
    }
    return $children;
}
...
function print_object_info($object)
{
    ...
    array_pop($lineage);
    echo count($lineage) ? ('<p>' . join(' -&gt; ', $lineage) . '</p>') : '<i>None</i>' . '</p>';
    echo '<h2>Methods<h2/>';
